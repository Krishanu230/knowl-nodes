import type {
  ...
  export function exportTableCellsToHTML(
    ...
    for (let x = rect != null ? rect.startX : 0; x < (rect != null ? rect.endX + 1 : firstRow.cells.length); x += 1) {
      const col = document.createElement('col');
      colGroup.append(col);
    }
  
    for (let y = rect != null ? rect.startY : 0; y < (rect != null ? rect.endY + 1 : rows.length); y += 1) {
      const row = rows[y];
      const { cells } = row;
      const rowElem = document.createElement('tr');
  
      for (let x = rect != null ? rect.startX : 0; x < (rect != null ? rect.endX + 1 : cells.length); x += 1) {
        const cell = cells[x];
  ...
  export function $createTableNodeWithDimensions(
    ...
    for (let y = 0; y < columnCount; y += 1) {
      ...
      for (let x = 0; x < rowCount; x += 1) {
        row.cells.push(createCell(includeHeaders === true && y === 0 ? 'header' : 'normal'));
      }
    }
    return new TableNode(rows);
  }